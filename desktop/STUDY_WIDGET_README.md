...
Progress is stored separately in the study-progress branch at progress/state.json. Windows uses Git Credential Manager; the Pages app uses a fine-grained GitHub token restricted to Hanyang with Contents read/write permission. Enter it under settings and press Connect and sync. The token stays in the current tab only.

Configure Pages yourself using the root static files. No deployment workflow is enabled by this update. This repository is public: progress, memos and revision history in study-progress are publicly readable. Never commit tokens.
